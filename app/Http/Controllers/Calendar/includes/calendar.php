<?php
	...
	**************************************************************************/
	...
	class AFFC2Calendar extends AFFC2Calendar_Extensions
	{
		###############################################################################################
		#### Properties
		###############################################################################################
		...
		*/
		public static function db_escape($str)
		{
			if(!isset($this))
			{
				$conn = AFFC2Calendar::connect(DB_HOST, DB_USERNAME, DB_PASSWORD, DATABASE);
		...
		*/
		public function json_transform($user_id)
		{
			...
			while($this->row = mysqli_fetch_array($this->result, MYSQLI_ASSOC))
			{
				...
				{
					if($user_id != "-1") {
						if(intval($this->row['user_id']) == intval($user_id))
						{} else {
		...
		*/
		public function get_event($id)
		{
			...
			{
				$result = mysqli_fetch_assoc($res);
				/* 0'dan başka id olursa detayını göstermiyor. 24.09.2021 - Kerem
				if(PUBLIC_PRIVATE_EVENTS == true)
				{
...
END:VCALENDAR';
		...
		* Returns true
		*/
		public function icalExport_all($uid)
		{
