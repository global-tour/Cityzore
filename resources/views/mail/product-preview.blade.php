...
<head>
    ...
    <!-- GOOGLE FONTS -->
    <title>Cityzore - Your Product is deactivated !</title>
    <style>
        body {
            margin-right: 15%;
            margin-left: 15%;
...
<div class="col-lg-12">
    ...
    <p>your product has been created but not yet active. For this reason, you can preview your product from the link below.</p>
    <p>
        <strong>Reference Number:</strong> {{$data['product']['referenceCode']}}
    </p>
    <p>
        <strong>Activity:</strong> @foreach($data['options'] as $o) {{json_decode($o, true)['title']}}<br> @endforeach
    </p>
    <p>
        <strong>Preview Link :</strong> <a href="{{env('APP_URL', 'https://cityzore.com').'/'.$data['product']['url']}} ">{{env('APP_URL', 'https://cityzore.com').'/'.$data['product']['url']}}</a><br>
    </p>
</div>
