...
{{--        <a href="{{url('/blog')}}" class="btn btn-default pull-right" style="margin-left: 10px;">Switch To CZ</a>--}}
        <select name="" id="" class="" style="width: 200px; display: inline-block; float: right; border: 1px solid; height: 37px;" onchange="window.location.href = this.value">
            <option value="/blog">Cityzore.com</option>
            <option value="/blogPCT">Pariscitytours.fr</option>
            <option value="/blogPCTcom" selected>Paris-city-tours.com</option>
            ...
            @foreach($blogPosts as $blogPost)
                <tr>
                  ...
                  <td>
                    ...
                    @else
                      <label for="" class="btn active btn-danger turn_action" data-id="{{$blogPost->id}}">Not Published</label>
                    @endif
                  </td>


                    <td>
                        <a href="{{url('/blog/'.$blogPost->id.'/editPCTcom')}}" class="sb2-2-1-edit"><i class="icon-cz-edit" aria-hidden="true"></i></a>&nbsp;
                        <a href="{{url('/blog/'.$blogPost->id.'/deletePCTcom')}}" class="sb2-2-1-edit"><i class="icon-cz-trash" aria-hidden="true"></i></a>
                    </td>
                </tr>
            @endforeach
            </tbody>
...
@include('panel-partials.scripts', ['page' => 'blog-index-pctcom'])
@include('panel-partials.datatable-scripts', ['page' => 'blog-index-pctcom'])
