                    ...
                    <div class="table-responsive table-desi" style="overflow-x: inherit;">
                        <table id="datatable" class="table">
                            ...
                            @foreach ($cities as $city)
                                <tr>
                                    <td>{{$i}}</td>
                                    <td style="width: 10%;">{{$city->name}}</td>
                                    <td>
                                        @foreach($languages as $lang)
                                            <p>{{$lang->name}}:
                                                @if((new \App\Http\Controllers\Admin\ConfigController)->isTranslated($city->id, $lang->id, 'city'))
                                    ...
                                    <td>
                                        <div class="col-lg-12" style="height: 30px;">
                                            <select data-city-id="{{$city->id}}" class="browser-default custom-select languageID" id="languageID">
...
@include('panel-partials.scripts', ['page' => 'citytranslations'])
