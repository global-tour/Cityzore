	...
	({
		...
		FullCalendarExt: function(options)
		{
			...
			var defaults =
			{
				...
				ajaxJsonFetch: '/guide/planning/cal_events?'+token,
				ajaxUiUpdate: '/guide/planning/cal_update?'+token,
				ajaxEventQuickSave: '/guide/planning/cal_quicksave?'+token,
				ajaxEventDelete: '/guide/planning/cal_delete?'+token,
				ajaxEventEdit: '/guide/planning/cal_edit_update?'+token,
				ajaxEventExport: '/guide/planning/cal_export?'+token,
				ajaxRepeatCheck: '/guide/planning/cal_check_rep_events?'+token,
				ajaxRetrieveDescription: '/guide/planning/cal_description?'+token,
				ajaxImport: '/guide/planning/importer?'+token,
				jsonConfig: '/calendar/includes/form.json',
			...
			$(opt.calendarSelector).fullCalendar(fc_extends);
					...
					$('#export-event').off().on('click', function(e)
					{
					 ...
					 $('#edit-event').off().on('click', function(e) {
						...
						$('#save-changes').off().on('click', function(e) {
							$('#user_id').val($('#guide-selection').val());
				...
				calendar.quickModal = function(start, end, allDay)
				{
					...
					$('#add-event').off().on('click', function(e) {
						if($('#guide-selection').val() == "") {
							alert('Guide selection is required');
							return false;
						}
						$('#user_id').val($('#guide-selection').val());
				...
				if(opt.filter == true)
				{
					$(opt.formFilterSelector).on('change', function(e)
					{
						 ...
						 $.post('/guide/planning/loader', construct, function(response)
						{
							$(opt.calendarSelector).fullCalendar('refetchEvents');
						});
